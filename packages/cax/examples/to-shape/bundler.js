...
for (var i = 0; i < 25; i++) {
	var circle = new _index2.default.Circle((i + 1) * 4, {
		lineWidth: 15,
		strokeStyle: '#113355'
	});
	...
	_index2.default.To.get(circle).to({ x: 150, y: 200 }, (0.5 + i * 0.04) * 1500, _index2.default.easing.bounceOut).start();
	stage.add(circle);
}

stage.on('click', function (evt) {
	stage.children.forEach(function (child, index) {
		_index2.default.To.get(child).to({ x: evt.stageX, y: evt.stageY }, (0.5 + index * 0.04) * 1500, _index2.default.easing.bounceOut).start();
	});
});
