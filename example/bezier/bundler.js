...
/***/ function(module, exports, __webpack_require__) {
	...
	var stage = new _index.Stage('#myCanvas');
	var curve = new _index.Graphics();
	stage.add(curve);
	var points = [100, 100, 200, 20, 300, 200, 337, 154];

	function renderCurve() {

	    curve.clear().beginPath().moveTo(points[0], points[1]).bezierCurveTo(points[2], points[3], points[4], points[5], points[6], points[7]).strokeStyle("#046ab4").lineWidth(2).stroke().beginPath().moveTo(points[0], points[1]).lineTo(points[2], points[3]).lineTo(points[4], points[5]).lineTo(points[6], points[7]).strokeStyle('red').stroke();
	}

	function addPoint(x, y, index) {
	    var graphics = new _index.Graphics();
	    graphics.cursor = 'move';
	    graphics.beginPath().arc(x, y, 5, 0, Math.PI * 2).lineWidth(3).stroke().fillStyle('green').fill();
	    stage.add(graphics);
